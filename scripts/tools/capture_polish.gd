...
const OUTPUT_DIR := "res://output/polish"
...
func _capture_martial_skill() -> void:
	_set_night(true)
	player.global_position = Vector2(40, 120)
	await _place_enemy(FIRE_LION_SCENE, Vector2(70, 0))
	await _settle()
	var enemy := world.get_node("Enemies").get_child(0)
	enemy.set_physics_process(false)
	enemy.global_position = player.global_position + Vector2(80, 0)
	player.last_facing_direction = Vector2.RIGHT
	player.selected_skill_slot = 2
	player.skill_caster.try_cast_slot(2)
	await create_timer(0.1).timeout
	_save_viewport("06-moon-cleave.png")
...
func _capture_pause_menu() -> void:
	...
	await _settle()
	_save_viewport("07-pause-menu.png")
	hud._unhandled_input(event)
